...
\scalebox{0.75}{
    \begin{prooftree}
        \hypo{\juicio [(\labsApp{x}{Bool}{y}, \labsApp{x}{Nat}{x})] }
        \hypo{\juicio \labsApp{x}{Nat}{x} : Nat \rightarrow Nat }
        \infer0[ (T-Var)]{ \juicio y : Nat (\text{ Osea }\sigma_k = Nat) }
        \infer3[( T-AppUnion )]{ \juicio
         ([ (\labsApp{x}{Bool}{y}, \labsApp{x}{Nat}{x})] y : Nat) : Nat }
    \end{prooftree}
}
